...
API_URL.URL_USERGUIDE = 'https://sinopacificukraine.com/support/manual/qt-dashcam/QT dashcam.pdf';
API_URL.URL_SUPPORT = "https://support.qt-dashcam.co/?name={0}&loginName={1}&email={2}&phone={3}&s={4}";
...
function showUserGuide(){
	var href = 'https://sinopacificukraine.com/support/manual/qt-dashcam/QT dashcam.pdf';
...
App.onPageInit('connect.wifi', function (page) {
	intervalForReply = setInterval(function () {				
				WifiWizard2.getConnectedSSID().then(response => {					
					...
					if (mySSID == TargetAsset.ASSET_IMEI) {	
...
function loadConnectPage(){
        mainView.router.load({
            ...
            context:{
				Login: TargetAsset.ASSET_IMEI,
				Password: TargetAsset.ASSET_IMEI.substr(-8)
            }
        }); 
...
function getRecordFront(resolve, reject) {	
	return new Promise((resolve, reject) => {
				...
				window.cordova.plugin.ftp.connect('192.168.0.105', 'quiktrak_biletskiy', '4eBcgg9S1N5I', function(ok) {
